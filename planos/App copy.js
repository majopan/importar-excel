...
const FloorPlan = () => {
  ...
  const [newPosition, setNewPosition] = useState({
    ...
    color: "#B0BEC5",
    borde: "",
    bordeDetalle: { top: false, right: false, bottom: false, left: false },
    piso: "PISO1",
    sede: "",
    servicio: "",
    dispositivos: "",
  ...
  const savePosition = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        showNotification(`Error al guardar: ${response.status}`, "error")
        throw new Error(`Error en la solicitud: ${response.status}`)
      }
  ...
  const deletePosition = async (id) => {
    try {
      ...
      if (!checkResponse.ok) {
        ...
        showNotification("Error: La posición no existe en el backend", "error")
        return
      }
      ...
      if (!response.ok) {
        showNotification(`Error en la eliminación: ${response.status}`, "error")
      ...
      try {
        ...
        for (let row = range.s.r; row <= range.e.r; row++) {
          for (let col = range.s.c; col <= range.e.c; col++) {
            ...
            if (!cell || !cell.v) continue // Omitir celdas vacías
            ...
            if (cell.s?.fill?.fgColor?.rgb) {
              cellColor = `#${cell.s.fill.fgColor.rgb}`
            }

            maxRows = Math.max(maxRows, row + 1)
            maxCols = Math.max(maxCols, col + 1)

            const id = `pos_${row}_${col}_${Date.now()}`
            const position = {
              id,
              nombre: cellValue,
              fila: row + 1,
            ...
            // Enviar al backend
            await fetch(`${API_URL}posiciones/`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
        ...
        setRows(maxRows)
        setColumns([...Array(maxCols)].map((_, i) => XLSX.utils.encode_col(i)))
        setPositions((prev) => ({ ...prev, ...newPositions }))

        showNotification("¡Se cargaron todas las posiciones correctamente!")
      } catch (error) {
  ...
  const handleBorderChange = (side) => {
    setNewPosition({
      ...newPosition,
      bordeDetalle: {
        ...newPosition.bordeDetalle,
        [side]: !newPosition.bordeDetalle[side],
      },
    })
  }
    ...
    return (
      <td
        ...
        style={{
          backgroundColor,
          color: textColor,
          borderTop: position?.bordeDetalle?.top ? "2px solid black" : "1px solid var(--border)",
          borderBottom: position?.bordeDetalle?.bottom ? "2px solid black" : "1px solid var(--border)",
          borderLeft: position?.bordeDetalle?.left ? "2px solid black" : "1px solid var(--border)",
          borderRight: position?.bordeDetalle?.right ? "2px solid black" : "1px solid var(--border)",
          position: "relative",
        }}
